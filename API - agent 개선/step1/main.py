import os, base64, json, re, uuid, datetime, asyncio
import httpx
...
def list_saved(user: User = Depends(require_auth), db: Session = Depends(get_db),
    ...
    total = db.query(SavedRecipe).filter_by(user_id=user.id).count()
    return {
        "success": True,
        "recipes": [
...
async def recognize(image: UploadFile = File(...)):
    ...
    raw = await _call_vision(b64, image.content_type)
...
@app.post("/api/recipe")
async def recipe_gen(req: RecipeReq, authorization: str = Header(None), db: Session = Depends(get_db)):
    ...
    raw = await _call_text(prompt)
    return {"success": True, "recipes": _parse_recipes(raw, req.ingredients)}

# ── 공통 유틸 ─────────────────────────────────────────────────────────────────
async def _call_vision(image_b64: str, content_type: str, retries: int = 3, wait: int = 10) -> str:
    """Vision 모델로 retry. 429면 지수 백오프로 재시도."""
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        for i in range(retries):
            if i:
                await asyncio.sleep(wait * (2 ** (i - 1)))
            try:
                r = await client.post(OR_URL, headers=hdrs, json=payload)
...
async def _call_text(prompt: str, temperature: float = 0.7) -> str:
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        for model in TEXT_MODELS:
            ...
            for attempt in range(2):
                if attempt:
                    await asyncio.sleep(10 * (2 ** (attempt - 1)))
                try:
                    r = await client.post(OR_URL, headers=hdrs, json=payload)
                except httpx.RequestError as e:
                    raise HTTPException(502, f"API 연결 오류: {e}")
